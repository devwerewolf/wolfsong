// This is the library code that each iframe "imports" (simulating bundled code)
// In a real scenario, each app would have this bundled into their build

class IframeStateSync {
  constructor(options = {}) {
    this.atoms = new Map();
...
function createChildClient(atomConfigs, options = {}) {
  ...
  atomConfigs.forEach(config => {
    // Use null as default if no initialValue provided
    const atomConfig = {
      key: config.key,
